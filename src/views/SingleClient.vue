...
<template>
  <div class="container-fluid">
    <b-row class="sub-header-row">
      <b-col lg="5">
        <span class="sub-header">{{ clients.filter(client => client.id == $route.params.id)[0].name }}</span>
      </b-col>
      <b-col lg="3">
        <b-input
          id="filter"
      ...
      <b-col lg="2" class="text-right">
        <download-csv
          ...
          :name="`${clients.filter(client => client.id == $route.params.id)[0].name}.csv`">
          Download CSV
        </download-csv>
      </b-col>
      <b-col lg="2">
...
export default {
  ...
  methods: {
    update(){
      this.$db.entries.where('client').equals(this.$route.params.id).toArray().then( entries => this.entries = entries )
    },
    add (){
      this.$db.entries.add({ 
        id: uuid(),
